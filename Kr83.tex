...
$^{83m}$Kr has been determined to be an ideal probe of the magnitude of the LUX TPC drift field. Detector efficiency effects cancel precisely in the S1a/S1b ratio, because the two decays occur at the same location in the detector, while the effect of the electric field on the S1a and S1b light yields is quite different, resulting in an excellent field observable. In this section we describe how we measure the S1a/S1b ratio, and how it relates to the absolute field determined by the COMSOL field map~\cite{luciespaper}.
...
We select $^{83m}$Kr events with exactly one S2 between 2000 to 60000~phd, and one or more S1 candidates greater than 10~phd occurring before the S2. We fit a double exponential to the S1 candidates and extract their pulse area and the time between decays. To ensure a good measurement of the S1a and S1b pulse areas, we require the time difference to be greater than 130~ns. The PMT waveforms from a typical event is shown in Fig.~\ref{fig:sumpod}.
...
To map the observed electric field in terms of S1a/S1b, we divide the  $^{83m}$Kr data from September 2015 
into voxels of XXX x YYY x ZZZ dimension. We fit a Gaussian to S1a/S1b ratio in each voxel and plot the mean vs detector radius and $z$ in 
Figure~\ref{fig:s1as1bmap}. The S1a/S1b ratio is observed to follow closely the electric field pattern from the COMSOL model for the same date. In Figure~\ref{fig:s1as1bvsfield} we plot the electric field from the model in each detector voxel vs S1a/S1b in the September 2015 data. A clear correlation is seen with the expected trend: higher electric fields reduce the light yield of the 32.1~keV decay while having only a modest effect on the 9.4~keV decay, resulting in a smaller value of S1a/S1b. The trend is replicated in other $^{83m}$Kr datasets collected at various times throughout the physics run under varying electric field conditions. We fit a function to the data shown in the Figure and find XXXXX.
